...
ViewVehiclesTxn::ViewVehiclesTxn(const std::shared_ptr<StorageAdapter>& storage_adapter,
  ...
  if (ids.size() != cities.size()) {
    SetError("Composite key length mismatch: vehicle ids != cities");
  }

  for (int i = 0; i < ids.size(); i++) {
    a_ids_.push_back(MakeInt32Scalar(ids[i]));
    a_cities_.push_back(MakeFixedTextScalar<64>(cities[i]));
  }
}
...
bool ViewVehiclesTxn::Read() {
  ...
  for (int i = 0; i < a_ids_.size(); i++) {
    auto res = vehicles_.Select({a_ids_[i], a_cities_[i]},
